...
class DropdownHelper:
    ...
    def __findLabelElement(page: Page, label: str) -> Locator:
        exact_match = page.locator("label, span").filter(
            has_text=DropdownHelper.__exactTextPattern(label)
        ).first
        if exact_match.count() > 0:
            return DropdownHelper.__ensureComponentVisible(exact_match, label, "label")

        partial_match = page.locator("label, span").filter(
            has_text=DropdownHelper.__partialTextPattern(label)
        ).first
        return DropdownHelper.__ensureComponentVisible(partial_match, label, "label")

    @staticmethod
    def __findControlByLabel(page: Page, label: str) -> Locator | None:
        label_element = DropdownHelper.__findLabelElement(page, label)
    ...
    def __findLabeledContainer(page: Page, label: str) -> Locator:
        ...
        label_pattern = DropdownHelper.__exactTextPattern(label)
    ...
    def __findCombobox(page: Page, label: str) -> Locator:
        exact_role_match = page.get_by_role(
            "combobox",
            name=DropdownHelper.__exactTextPattern(label),
        ).first
        ...
        partial_role_match = page.get_by_role(
            "combobox",
            name=DropdownHelper.__partialTextPattern(label),
        ).first
        if partial_role_match.count() > 0:
            return partial_role_match

        control = DropdownHelper.__findControlByLabel(page, label)
        ...
        return DropdownHelper.__findLabeledContainer(page, label).locator(
    ...
    def __waitUntilDropdownEditable(
        ...
        combobox = DropdownHelper.__ensureComponentVisible(
            DropdownHelper.__findCombobox(page, label),
            label,
            "dropdown",
        )
        ...
        while time.monotonic() < deadline:
            if DropdownHelper.__isDropdownEditable(combobox):
                return combobox
    ...
    def selectDropdownIfEditable(
        ...
        if not DropdownHelper.__isDropdownEditable(combobox):
            print(
        ...
        controls_deadline = time.monotonic() + max(min(poll_interval_seconds, 1.0), 0.25)
        ...
        if dropdown_option_id:
            ...
            option_locators.append(
                scoped_options.filter(has_text=DropdownHelper.__exactTextPattern(value)).first
            )
            option_locators.append(
                scoped_options.filter(has_text=DropdownHelper.__partialTextPattern(value)).first
            )

        option_locators.append(
            page.get_by_role("option", name=DropdownHelper.__exactTextPattern(value)).first
        )
        option_locators.append(
            page.get_by_role("option", name=DropdownHelper.__partialTextPattern(value)).first
        )
        option_locators.append(
            page.locator('[role="option"], li').filter(
                has_text=DropdownHelper.__partialTextPattern(value)
            ).first
        )
        ...
        print(
            f"Skipping '{label}' because option '{value}' was not available in time."
        )
        return False
